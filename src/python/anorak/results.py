...
class Result:
    ...
    def __str__(self):
        ...
        home_scorers = "[" + ",".join([str(g) for g in self.home_goals]) + "]" if self.home_goals else ""
        away_scorers = "[" + ",".join([str(g) for g in self.away_goals]) + "]" if self.away_goals else ""
    ...
    def __cmp__(self, other):
        """Results are ordered primarily by date.  Results from the same day
        are sorted alphabetically by home team."""
...
class Goal:
    ...
    def __str__(self):
        """Returns scorer name, the goal minute, and (optionally) the goal type
        ('p' for penalty, 'o' for own goal)."""
        return "".join([self.scorer,
                        str(self.minute),
                        self.goal_type if self.goal_type else ""])
...
def parse_rlt(rlt_path, player_aliases = {}):
    """Read an RLT file and return a list of results and a list of metadata
    records."""
    with open(rlt_path) as rlt:
        ...
        while line:
            if line[0].isdigit():
                ...
                (home_goals, home_scorers) = _parse_score(fields[2],
                                                          fields[1],
                                                          fields[3],
                                                          player_aliases)
                (away_goals, away_scorers) = _parse_score(fields[4],
                                                          fields[3],
                                                          fields[1],
                                                          player_aliases)
                results.append(Result(date,
                                      fields[1],
                                      home_goals,
                                      fields[3],
                                      away_goals,
...
def _parse_score(text, team, opposition, player_aliases):
    """Parse the score field of an RLT record and return a tuple containing the
    scorer and scorers."""
...
def write_rlt(rlt_path, results, metadata):
    """Write a new RLT file with the specified file name containing the
    specified results and metadata."""
